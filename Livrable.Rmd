...
On s'intéresse ici au stock aggrégé de détenus en France. On peut commencer par observer l'évolution du nombre de détenus en France lors des 3 derniers mois et comparer ces observations aux prévisions faites par les modèles X13.
...
Les résulats sont plutot bons, en effet, les prévisions ont le même sens de variation que les chiffres observés. Pour être plus précis sur ces chiffres, on a pensé à décomposer la série détenus de différentes manières pour prendre en compte les saisonnalités propres de certaines composantes du nombre de détenus.

## Première décomposition : Condamnés/Prévenus

Une première décomposition de nombre de détenus peut-être la décomposition en condamnés et en prévenus le nombre de détenus totals étant la somme de ces deux séries. La répartition des effectifs est à peu près de trois quarts de condamnés et un quarts de prévenus.
...
On observe maintenant les résulats d'un modèle X13 sur la série des prévenus.
...
On observe maintenant les résulats d'un modèle X13 sur la série des détenus incarcérés dans un autre type de quartier pénitentiaire.
...
Enfin une dernière décomposition permettant de prendre en compte des séries ayant des saisonnalités propre est la décompostion par Direction Interrégionale. Par soucis de clarté, on affiche uniquement les résultats agrégés et non les résultats de chaques modèles pour les dix DI. 
...
df <-data.frame(rep(date, 4),
            ...
            pre_DISP),
            group = c(
                      rep("Prévision détenus", 24),
                      rep("Prévisions décomposition Condamnés/Prévenus", 24),
                      rep("Prévisions MA/Autres", 24),
                      rep("Prévisions DISP", 24)))
